...
def objective(trial: optuna.Trial, 
    ...
    initialization_multiplier = trial.suggest_float("initialization_multiplier", 1e-4, 2)
...
def objective_local_gates(trial: optuna.Trial, 
              grid_connectivity, 
              num_qubits,
              base_sigma, 
              train_ds,
    ...
    gates = local_gates(num_qubits, 2)
    iqp_circ = iqp.IqpSimulator(num_qubits, gates, device='lightning.qubit')
    sigma = base_sigma * sigma_multiplier
    params_init = initialize_from_data(gates, train_ds) * initialization_multiplier
